...
$('doSignup').onclick=async()=>{
  ...
  if(pw.length<6){m.textContent='비밀번호는 6자 이상.';return;}
  if(pw!==pw2){m.textContent='비밀번호가 일치하지 않습니다.';return;}
  if(!/^\d{4}$/.test(a)){m.textContent='계좌 비밀번호는 숫자 4자리.';return;}
...
$('pmSaveSec').onclick=async()=>{
  ...
  if(a&&!/^\d{4}$/.test(a)){m.style.color='var(--down)';m.textContent='계좌 비밀번호는 숫자 4자리.';return;}
...
function renderViewHist(){
  ...
  el.innerHTML=`<div class="sh-head"><span>최근 본 종목</span><button id="vhClear">전체 삭제</button></div>
    <div class="sh-chips">${viewHist.map((h,i)=>`<span class="sh-chip" data-i="${i}">${stockLogo(h.code,h.name,'xs')}<b>${h.name}</b><i class="sh-x" data-x="${i}">✕</i></span>`).join('')}</div>`;
  el.querySelectorAll('.sh-chip').forEach(c=>c.onclick=(e)=>{
    if(e.target.classList.contains('sh-x'))return;
    const h=viewHist[+c.dataset.i]; if(h){ensureStock(h.code,h.name,h.market);openTrade(h.code);}
  });
...
function renderUsFxCard(){
  ...
  box.innerHTML=`<div class="card us-card"><div class="us-sec"><b>💱 실시간 환전</b>
    ...
    <button class="us-submit buy" id="usFxGo">${fx?'환전 실행':'환율 확인 중 · 눌러서 다시 시도'}</button>
...
function renderUsLive(){ if(currentView!=='us')return;
  ...
  /* [v4.34] 환율 표시는 여러 곳에 있다 — 첫 요소만 갱신하면 환전 카드가 옛 값으로 남는다 */
  const fx=usFx();
  if(fx){const el=document.querySelector('#usTop .us-fx b'); if(el)el.textContent=KRW(Math.round(fx))+'원';}
}
