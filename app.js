...
function acctGateShow(what){
  ...
  ov.querySelector('#agGo').onclick=()=>{ close();
    ...
    setTimeout(()=>{ try{
      const b=$('aeFirstOpen')||$('acctAddBtn');
      if(b)b.click();
      else { const el=$('acctGuide'); if(el)el.scrollIntoView({behavior:'smooth',block:'center'}); }
    }catch(e){} },300);
  };
...
function renderAcctBar(){
  ...
  if(!acctOpened()){
    ...
    /* ══ [v5.03] 최초 개설도 같은 절차를 밟는다 ═══════════════════════════════
       [무엇이 잘못됐나] 계좌가 하나도 없을 때는 '옛 화면'이 떴다. 종류·예수금·비밀번호를
       한 화면에서 받고 곧바로 doAcctOpen() 을 부르는데, 그 함수는 절차 시트가 채워 두는
       aeForm 을 전제로 한다. 절차를 거치지 않았으니 aeForm 이 null 이라
       'Cannot read properties of null (reading cash)' 오류가 났다(첨부 사진).
       게다가 신청인 정보·약관 동의를 건너뛰어 두 번째 계좌와 절차가 달랐다.
       → 안내만 남기고, 개설은 두 번째 계좌와 똑같이 4단계 시트로 진행한다. */
    guide.innerHTML=`<div class="panel acct-empty">
      ...
      <div class="ae-steps3">
        <div class="ae-s3"><i>1</i><span>계좌 선택</span></div>
        <div class="ae-s3"><i>2</i><span>신청인 정보</span></div>
        <div class="ae-s3"><i>3</i><span>약관 동의</span></div>
        <div class="ae-s3"><i>4</i><span>예수금·비밀번호</span></div>
      </div>
      <button class="modal-btn" id="aeFirstOpen">계좌 개설 시작하기</button>
      <div class="ae-note">모의 계좌입니다 · 실제 금융거래가 아니며 개인정보를 요구하지 않습니다.</div>
    </div>`;
    {const b=$('aeFirstOpen'); if(b)b.onclick=()=>openAcctOpenSheet();}
...
function unlockApp(){
  /* ══ [v8.8] 순서가 또 뒤집혀 있었다 ═══════════════════════════════════════
     [영상에서 본 것] '확인 중…' 이 떠 있는데 배경에 이미 홈이 그려져 있었다.
     [왜] 이 함수가 ① 로그인 창을 닫고 ② 그 다음에 입장 화면을 띄웠다.
     ①과 ② 사이에 아무것도 없는 순간이 생겨 홈이 그대로 드러났다.
     [고침] 입장 화면을 '먼저' 덮고, 그것이 화면을 가린 뒤에 로그인 창을 닫는다.
     입장 화면은 z-index 2000000 이라 로그인 창(200)보다 위에 있으므로
     먼저 띄워도 어색하지 않고, 오히려 자연스럽게 이어진다. */
  const closeGate=()=>{
    const g=$('authGate');
...
async function oaConsume(){
  ...
  /* [v7.4] 표는 이제 쿠키로 온다 — 주소의 t 는 없어도 된다 */
  try{
    const r=await fetch('/api/oauth/google?claim=1',
      {cache:'no-store',credentials:'same-origin'});
    const j=await r.json();
...
function drawSpark(cv,hist,dir){
  /* ══ [v8.8] 값이 모두 같으면 그리지 않는다 ═══════════════════════════════
     같은 값만 이어 붙이면 완전한 수평선이 된다. 그건 '움직이지 않았다'는
     잘못된 인상을 준다 — 이력을 못 받은 것과 구분되지 않는다. */
  try{
    if(Array.isArray(hist)&&hist.length>=2){
      const a=hist.map(Number).filter(v=>isFinite(v));
      if(a.length>=2){
        const mn=Math.min(...a), mx=Math.max(...a);
        if(mx-mn<Math.abs(mx)*1e-9){ try{ if(cv)cv.style.display='none'; }catch(e){} return; }
        try{ if(cv)cv.style.display=''; }catch(e){}
      }
    }
  }catch(e){}
  /* ══ [v8.8] 값이 모두 같으면 그리지 않는다 ═══════════════════════════════
  ...
  try{
    if(Array.isArray(arguments[1])&&arguments[1].length>=2){
      const a=arguments[1].map(Number).filter(v=>isFinite(v));
      if(a.length>=2){
        const mn=Math.min(...a), mx=Math.max(...a);
        if(mx-mn<Math.abs(mx)*1e-9){ try{ if(arguments[0])arguments[0].style.display='none'; }catch(e){} return; }
        try{ if(arguments[0])arguments[0].style.display=''; }catch(e){}
      }
    }
  }catch(e){}
...
function mktBadge(key,dayBasis){
  ...
  const lab=(key==='BTC'||key==='ETH')?'24시간'
    ...
    /* [v8.8] 코스피200 선물(주간)에 '야간 거래 중'을 붙이던 것을 바로잡는다.
       주간 선물은 15:45 에 끝나고 거기서 멈춘다. 밤에 움직이는 것은
       '코스피200 야간선물'이라는 별도 종목이다. 두 개를 섞으면 안 된다. */
    :(key==='K200F')?(open?'장중':'주간 마감')
...
function paintClan(){
  ...
  if(clanTab==='league'){
    ...
    $('clanMsgSave').onclick=async()=>{
      ...
      const _hp=allHoldingPerf().sort((a,b)=>b.rate-a.rate);
      const r=await clanCall('sync',{msg:$('clanMsg').value,rate:monthPerf().rate,ym:monthPerf().ym,
        tr:allTrades().length, hold:_hp.length,
        holds:_hp.slice(0,12).map(x=>({n:x.name,r:x.rate,us:x.us})),
        best:_hp[0]?{n:_hp[0].name,r:_hp[0].rate}:null,
        worst:_hp.length>1?{n:_hp[_hp.length-1].name,r:_hp[_hp.length-1].rate}:null});
...
function openMemberCard(id){
  ...
  openLiteGate(m.name+' 님',`
    ...
    ${(m.best||m.worst)?`<div class="mc-bw">
      ${m.best?`<div class="mc-b up"><span>🥇 수익률 BEST</span><b>${htmlEsc(m.best.n)}</b><i class="num up">${m.best.r==null?'—':pctS(m.best.r)}</i></div>`:''}
      ${m.worst?`<div class="mc-b down"><span>🥶 수익률 WORST</span><b>${htmlEsc(m.worst.n)}</b><i class="num down">${m.worst.r==null?'—':pctS(m.worst.r)}</i></div>`:''}
    </div>`:''}
...
function clanAutoSync(force){
  ...
  /* [v5.03] 자동 동기화도 전 계좌 합계로 */
  const hp=allHoldingPerf().sort((a,b)=>b.rate-a.rate);
  clanCall('sync',{rate:mp.rate,ym:mp.ym,tr:allTrades().length,hold:hp.length,
    holds:hp.slice(0,12).map(x=>({n:x.name,r:x.rate,us:x.us})),
    best:hp[0]?{n:hp[0].name,r:hp[0].rate}:null,
    worst:hp.length>1?{n:hp[hp.length-1].name,r:hp[hp.length-1].rate}:null}).then(r=>{
...
/* ══ [v5.03] 클랜·친구에는 '모든 계좌를 합친' 값을 쓴다 ═══════════════════════
   [무엇이 잘못됐나] 지금까지는 '활성 계좌' 하나만 보고 수익률·보유·체결 수를 보냈다.
   그래서 내 계좌에서 계좌를 바꾸면 클랜에 보이는 내 기록이 통째로 달라졌다.
   계좌는 내가 나눠 쓰는 주머니일 뿐, 남에게 보이는 성적은 전부를 합친 것이어야 한다.
   [고침] 활성 계좌(지금 화면의 값) + 나머지 계좌 장부를 더해 하나로 만든다. */
function allBooks(){
  const out=[];
  try{
    ...
    for(const a of list){
      if(a.id===acctActive){
        out.push({id:a.id,type:a.type,cash,usdCash,holdings:holdings||[],
          tradeLog:tradeLog||[],tradeArchive:tradeArchive||{}});
      }else{
        const b=acctBooks[a.id]||{};
        out.push({id:a.id,type:a.type,cash:+b.cash||0,usdCash:+b.usdCash||0,
          holdings:Array.isArray(b.holdings)?b.holdings:[],
          tradeLog:Array.isArray(b.tradeLog)?b.tradeLog:[],
          tradeArchive:b.tradeArchive||{}});
      }
    }
  }catch(e){}
  return out;
...
/* 전 계좌 합계 총자산(원화 환산) */
function allAssetsNow(){
  const num=(v)=>{const n=Number(v);return isFinite(n)?n:0;};
  let fx=0; try{ fx=num(usFx()); }catch(e){}
  let tot=0;
  for(const b of allBooks()){
    tot+=Math.max(0,num(b.cash));
    if(fx>0)tot+=Math.max(0,num(b.usdCash))*fx;
...
/* 전 계좌 보유 종목을 하나로 합친다(같은 종목은 수량을 더하고 평단을 다시 계산) */
function allHoldings(){
  const map={};
  for(const b of allBooks())for(const h of (b.holdings||[])){
    if(!h||!h.code)continue;
    const q=Number(h.qty)||0; if(q<=0)continue;
    const cur=map[h.code]||(map[h.code]={code:h.code,qty:0,cost:0,us:h.us?1:0});
    cur.qty+=q; cur.cost+=(Number(h.avg)||0)*q;
  }
  return Object.values(map).map(x=>({code:x.code,qty:x.qty,avg:x.qty?x.cost/x.qty:0,us:x.us}));
}
/* 전 계좌 체결 기록 */
function allTrades(){
  const out=[];
  for(const b of allBooks()){
    out.push(...(b.tradeLog||[]));
    try{ Object.values(b.tradeArchive||{}).forEach(v=>{ if(Array.isArray(v))out.push(...v); }); }catch(e){}
  }
  return out;
}
/* 보유 종목별 수익률 — 클랜에 보여 줄 BEST/WORST 를 뽑는 데 쓴다 */
...
function monthPerf(){
  const ym=kstMonth();
  const cur=allAssetsNow();          // [v5.03] 활성 계좌가 아니라 전 계좌 합계
...
/* ══ [v5.03] 검색·열람 기록을 한곳에 모은다 ═══════════════════════════════════
   국내·해외를 가리지 않고, 검색해서 들어갔든 목록에서 눌러 들어갔든 여기에 남는다.
   (앞서 '최근 본 종목'을 없애면서 이 함수도 함께 지워졌던 것을 다시 세운다) */
function pushHist(code,name,market){
  if(!code)return;
  srchHist=srchHist.filter(x=>x&&x.code!==code);
  srchHist.unshift({code,name:name||code,market:market||'',t:Date.now()});
  srchHist=srchHist.slice(0,20); saveHist();
  try{ if(currentView==='search')renderHist(); }catch(e){}
}
function pushHistQ(q){
  q=String(q||'').trim(); if(!q)return;
  srchHist=srchHist.filter(x=>x&&x.q!==q);
...
/* ══ [v5.03] '최근 본 종목'은 걷어냈다 ═══════════════════════════════════════
   ...
   종목을 열면 검색 기록에도 남으므로 국내·해외가 한곳에 모인다. */
function renderViewHist(){ const el=$('viewHist'); if(el)el.hidden=true; }
function renderHist(){
...
/* ══ [v8.9] 매매 타점 엔진 ═══════════════════════════════════════════════════
   ...
   매집 분석의 결론은 결국 '어디서 담고 어디서 덜어내나' 여야 한다.
   [무엇을 근거로 삼나] 와이코프 국면 전환의 고전적 신호를 그대로 옮긴다.
     · 매수 — ① 스프링 복귀(하단 이탈 후 되돌림) ② 흡수 캔들 뒤 눌림
              ③ 박스 상단 대량 돌파(SOS) ④ 돌파 후 저거래 되돌림(LPS)
     · 매도 — ① 박스 상단에서 대량인데 종가가 아래쪽(UT, 가짜 돌파)
              ② OBV 는 꺾이는데 가격만 오르는 어긋남 ③ 급등 후 대량 음봉
...
function acSignals(rawBars,r){
  ...
  const dedup=(arr)=>{
    const m={};
    for(const s2 of arr){ if(!m[s2.i]||m[s2.i].w<s2.w)m[s2.i]=s2; }
  ...
  const rb=recent(B), rs=recent(S);
  const last=bars[n-1];
  let now=null;
  if(rs.length&&(!rb.length||rs[rs.length-1].i>=rb[rb.length-1].i)){
    const t=rs[rs.length-1];
    now={side:'sell',kind:t.kind,why:t.why,px:t.px,ago:n-1-t.i};
  }else if(rb.length){
    const t=rb[rb.length-1];
    now={side:'buy',kind:t.kind,why:t.why,px:t.px,ago:n-1-t.i};
  }else{
    now={side:'wait',kind:'대기',
      why:r.total>=70?'점수는 높지만 최근 5거래일 안에 들어갈 자리는 나오지 않았습니다'
        :'뚜렷한 자리가 아직 없습니다. 박스 하단 되돌림이나 대량 돌파를 기다리세요',
      px:last.c,ago:0};
  }

  /* ── 되짚어 보기 ── 지난 신호가 어땠는지 (10거래일 뒤 성과) */
  const review=[];
  for(const t of B){
    const j=Math.min(n-1,t.i+10);
    if(j>t.i)review.push({side:'buy',i:t.i,kind:t.kind,ret:(c[j]-t.px)/t.px*100});
  }
  for(const t of S){
    const j=Math.min(n-1,t.i+10);
    if(j>t.i)review.push({side:'sell',i:t.i,kind:t.kind,ret:(t.px-c[j])/t.px*100});
  }
  const done=review.filter(x=>x.i<=n-11);
  const wins=done.filter(x=>x.ret>0).length;
  const stat=done.length?{n:done.length,win:Math.round(wins/done.length*100),
    avg:+(done.reduce((a,x)=>a+x.ret,0)/done.length).toFixed(2)}:null;

  /* ── 가격 계획 ── 박스와 변동성으로 잡는다 */
  const atr=(()=>{let s2=0,k=0;
    for(let i=n-14;i<n;i++){if(i<1)continue;
      s2+=Math.max(h[i]-l[i],Math.abs(h[i]-c[i-1]),Math.abs(l[i]-c[i-1]));k++;}
    return k?s2/k:last.c*0.02;})();
  const plan={
    buy1:box.lo?Math.max(box.lo,last.c-atr*1.2):last.c-atr*1.2,
    buy2:box.lo?box.lo*0.99:last.c-atr*2.2,
    stop:box.lo?box.lo*0.965:last.c-atr*3,
    tgt1:box.hi?box.hi:last.c+atr*2,
    tgt2:box.hi?box.hi+(box.hi-box.lo)*0.6:last.c+atr*4,
    atr:+atr.toFixed(2)
  };
  plan.rr=+(((plan.tgt1-last.c)/Math.max(1e-9,last.c-plan.stop))).toFixed(2);
  return {buys:B,sells:S,now,stat,plan,px:last.c};
}
/*AC-CORE-END*/
/* ── UI ── */
...
async function acAnalyze(code){
  ...
  /* [v8.9] 타점을 계산해 차트와 요약에 함께 쓴다 */
  try{ r._sig=acSignals(bars,r); }catch(e){ r._sig=null; }
  try{ acRenderSignals(r._sig,code); }catch(e){}
  requestAnimationFrame(()=>acDrawChart(acBars(bars),r));
}
/* ══ [v8.9] 타점 요약 · 매매 계획 · 되짚어 보기 ═══════════════════════════════
   차트에 찍은 자리를 말로 풀어 준다. 무엇을 언제 왜 해야 하는지가 한눈에 보이게. */
function acRenderSignals(sg,code){
  const box=$('acSigBox'); if(!box||!sg)return;
  const N=(v)=>KRW(Math.round(v));
  const now=sg.now||{};
  const tone=now.side==='buy'?'buy':now.side==='sell'?'sell':'wait';
  const label=now.side==='buy'?'지금은 매수 자리':now.side==='sell'?'지금은 덜어낼 자리':'지금은 기다릴 때';
  const icon=now.side==='buy'?'▲':now.side==='sell'?'▼':'⏸';
  const p=sg.plan||{};
  const st=sg.stat;
  box.innerHTML=`
    <div class="ac-now ${tone}">
      <div class="ac-now-h"><i>${icon}</i><b>${label}</b>
        <span class="ac-now-k">${htmlEsc(now.kind||'')}</span>
        ${now.ago>0?`<span class="ac-now-a">${now.ago}거래일 전 신호</span>`:''}</div>
      <div class="ac-now-w">${htmlEsc(now.why||'')}</div>
    </div>

    <div class="ac-sec">매매 계획 <span>박스와 변동성(ATR ${N(p.atr||0)}원)으로 잡은 값</span></div>
    <div class="ac-plan">
      <div class="ac-pl buy"><span>1차 매수</span><b class="num">${N(p.buy1||0)}</b>
        <i>${(((p.buy1-sg.px)/sg.px)*100).toFixed(1)}%</i></div>
      <div class="ac-pl buy"><span>2차 매수</span><b class="num">${N(p.buy2||0)}</b>
        <i>${(((p.buy2-sg.px)/sg.px)*100).toFixed(1)}%</i></div>
      <div class="ac-pl tgt"><span>목표 1</span><b class="num">${N(p.tgt1||0)}</b>
        <i class="up">+${(((p.tgt1-sg.px)/sg.px)*100).toFixed(1)}%</i></div>
      <div class="ac-pl tgt"><span>목표 2</span><b class="num">${N(p.tgt2||0)}</b>
        <i class="up">+${(((p.tgt2-sg.px)/sg.px)*100).toFixed(1)}%</i></div>
      <div class="ac-pl stop"><span>손절</span><b class="num">${N(p.stop||0)}</b>
        <i class="down">${(((p.stop-sg.px)/sg.px)*100).toFixed(1)}%</i></div>
      <div class="ac-pl rr"><span>손익비</span><b class="num">${p.rr>0?p.rr+' : 1':'—'}</b>
        <i>${p.rr>=2?'좋음':p.rr>=1.5?'보통':'낮음'}</i></div>
    </div>
    <div class="ac-note">손익비는 <b>목표1까지 얻을 폭 ÷ 손절까지 잃을 폭</b>입니다.
      2 이상이면 세 번 중 한 번만 맞아도 손해가 아닙니다.</div>

    ${st?`<div class="ac-sec">되짚어 보기 <span>지난 신호가 10거래일 뒤 어땠나</span></div>
    <div class="ac-back">
      <div><span>신호 수</span><b class="num">${st.n}회</b></div>
      <div><span>맞은 비율</span><b class="num ${st.win>=60?'up':st.win<=40?'down':''}">${st.win}%</b></div>
      <div><span>평균 성과</span><b class="num ${st.avg>0?'up':'down'}">${st.avg>0?'+':''}${st.avg}%</b></div>
    </div>
    <div class="ac-note">이 종목의 과거 자료로만 따진 값입니다. 표본이 적으면 흔들리고,
      과거가 앞날을 보장하지 않습니다.</div>`:''}

    ${(sg.buys.length||sg.sells.length)?`
    <div class="ac-sec">최근 신호 <span>차트의 BUY·SELL 표시</span></div>
    <div class="ac-sigs">
      ${[...sg.buys.slice(-4).map(t=>({...t,s:'buy'})),
         ...sg.sells.slice(-4).map(t=>({...t,s:'sell'}))]
        .sort((a,b)=>b.i-a.i).slice(0,6).map(t=>`
        <div class="ac-sig ${t.s}">
          <span class="ac-sig-t">${t.s==='buy'?'BUY':'SELL'}</span>
          <b>${htmlEsc(t.kind)}</b>
          <i class="num">${N(t.px)}</i>
          <em>${htmlEsc(t.why)}</em>
        </div>`).join('')}
    </div>`:''}`;
}
function acDrawChart(bars,r){
  ...
  /* ══ [v8.9] BUY · SELL 타점 ═══════════════════════════════════════════════
     점수만 보여 주면 '언제 사라는 건데?' 가 남는다. 자리를 직접 찍어 준다.
     매수는 캔들 아래에서 위를 가리키고, 매도는 위에서 아래를 가리킨다. */
  try{
    const sg=r._sig;
    if(sg){
      const mark=(i,side)=>{
        if(i<0||i>=vn)return;
        const X=px(i), buy=(side==='buy');
        const Y=buy?py(view[i].l)+22:py(view[i].h)-22;
        const col=buy?'#12b76a':'#e5484d';
        /* 삼각형 */
        x2.beginPath();
        if(buy){ x2.moveTo(X,Y-9); x2.lineTo(X-6,Y+1); x2.lineTo(X+6,Y+1); }
        else   { x2.moveTo(X,Y+9); x2.lineTo(X-6,Y-1); x2.lineTo(X+6,Y-1); }
        x2.closePath(); x2.fillStyle=col; x2.fill();
        /* 글자 */
        x2.font='900 9px Pretendard,sans-serif'; x2.fillStyle=col; x2.textAlign='center';
        x2.fillText(buy?'BUY':'SELL',X,buy?Y+12:Y-8);
      };
      sg.buys.forEach(t=>mark(t.i-off,'buy'));
      sg.sells.forEach(t=>mark(t.i-off,'sell'));
      /* 가격 계획선 */
      if(sg.plan){
        const line=(v,col,lb)=>{
          const Y=py(v); if(!isFinite(Y)||Y<10||Y>H-10)return;
          x2.setLineDash([3,3]); x2.strokeStyle=col; x2.lineWidth=1;
          x2.beginPath(); x2.moveTo(px(0),Y); x2.lineTo(px(vn-1),Y); x2.stroke();
          x2.setLineDash([]);
          x2.font='700 9px Pretendard,sans-serif'; x2.fillStyle=col; x2.textAlign='left';
          x2.fillText(lb,px(0)+3,Y-3);
        };
        line(sg.plan.tgt1,'rgba(18,183,106,.75)','목표1');
        line(sg.plan.buy1,'rgba(59,130,246,.75)','1차 매수');
        line(sg.plan.stop,'rgba(229,72,77,.75)','손절');
      }
    }
  }catch(e){}
}
  ...
  for(let i=0;i<codes.length;i+=CHUNK){
    await Promise.all(codes.slice(i,i+CHUNK).map(async c=>{
      try{const bars=await acFetchDaily(c);const r=acScore(bars,null);
        if(r.ok){ try{ r._sig=acSignals(bars,r); }catch(e2){}
          out.push({c,r}); }}catch(e){}
      done++;
  ...
  body.innerHTML=out.length?`<table class="ac-tbl"><thead><tr><th>종목</th><th>매집 점수</th><th>단계</th><th>지금</th></tr></thead><tbody>
    ${out.map(({c,r})=>{const st=byCode[c]||{};
      ...
      const sig=nw?`<span class="ac-tag ${nw.side}">${nw.side==='buy'?'매수':nw.side==='sell'?'매도':'대기'}</span>`
        +(nw.side!=='wait'?`<small>${htmlEsc(nw.kind)}</small>`:'')
        :[r.spring?'🌀':'',r.stars.length?`★${r.stars.length}`:''].filter(Boolean).join(' ')||'—';
...
/* ══ [v5.03] 개별주 레버리지 ETF는 기초자산 로고를 따른다 ═══════════════════
   ...
   (Direxion·GraniteShares) 로고로 나와, 같은 테슬라를 따르는 상품인데도 로고가
   제각각이었다. '나스닥100 3X'와 '나스닥100 2X'도 운용사가 달라 모양이 달랐다.
   [고침] 개별 종목을 따르는 레버리지 ETF는 그 종목의 로고를 그대로 쓴다.
   지수를 따르는 것(나스닥100·S&P500)은 지수끼리 같은 로고를 쓰도록 묶는다. */
var US_LEV_BASE={
  /* 개별 종목 레버리지 → 기초 종목 */
  TSLL:'TSLA', TSLQ:'TSLA', TSLS:'TSLA', TSDD:'TSLA',
  NVDL:'NVDA', NVDX:'NVDA', NVD:'NVDA', NVDQ:'NVDA', NVDU:'NVDA',
  CONL:'COIN', CONI:'COIN',
  AAPU:'AAPL', AAPD:'AAPL',
  MSFU:'MSFT', MSFD:'MSFT',
  AMZU:'AMZN', AMZD:'AMZN',
  ...
  SPAL:'SPCX', SNK:'SPCX',
  SMCL:'SMCI', SMCX:'SMCI',
  AVL :'AVGO', MUU:'MU', ARMU:'ARM',
  /* 지수 레버리지 → 대표 ETF 로고로 통일 (같은 지수는 같은 모양) */
  TQQQ:'QQQ', SQQQ:'QQQ', QLD:'QQQ', QID:'QQQ', PSQ:'QQQ',
  UPRO:'SPY', SPXL:'SPY', SPXU:'SPY', SPXS:'SPY', SSO:'SPY', SDS:'SPY',
  SOXL:'SOXX', SOXS:'SOXX', USD:'SOXX',
  BITX:'IBIT', BITU:'IBIT'
};
function usLgUrls(t){
  /* 기초자산이 정해진 레버리지 ETF는 그 종목의 로고를 그대로 쓴다 */
  {const b=US_LEV_BASE[t]; if(b&&b!==t)return usLgUrlsRaw(b);}
...
function openUS(t){ if(!usMeta[t])return;
  ...
  /* [v5.03] 해외도 같은 검색 기록에 남긴다 */
  safeRun('openUS:view',()=>{ const m=usMeta[t]||{}; pushHist(t,m.kr||t,'US'); });
  showView('ustrade');
}
...
function renderUsCons(el){
  ...
  if(!html.includes('gauge-wrap')&&!c.target&&!cells.length){
    /* [v5.03] 왜 비었는지 알 수 있게 — 자료가 없는 것과 받아오지 못한 것을 구분한다 */
    const dg=(info&&info.diag)||[];
    const failed=dg.some(x=>/yh-(crumb|sum)/.test(x)&&!/ok/.test(x));
    el.innerHTML=usNoData(
      failed?'컨센서스를 받아오지 못했습니다':'컨센서스 자료가 없습니다',
      failed?'시세 제공처 연결이 일시적으로 막혔습니다. 잠시 뒤 다시 열어 보세요.'
            :'소형주·신규 상장·ETF 는 애널리스트 의견과 목표주가가 없는 경우가 많습니다.');
    return;
  }
...
function renderUsFin(el){
  ...
  if(!html){
    ...
    const failed=dg.some(x=>/yh-(crumb|sum)/.test(x)&&!/ok/.test(x));
    el.innerHTML=usNoData(
      failed?'재무 정보를 받아오지 못했습니다':'재무 정보가 없습니다',
      failed?'시세 제공처 연결이 일시적으로 막혔습니다. 잠시 뒤 다시 열어 보세요.'
            :'ETF·ETN 은 재무제표가 없고, 신규 상장 종목은 아직 공시가 쌓이지 않았습니다.');
    return; }
  el.innerHTML=html;
}
