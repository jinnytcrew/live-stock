...
var chart_default = async (req2) => {
  ...
  try {
    ...
    if (IDX_SYM[code]) {
      ...
      /* ══ [v12.6] 3분·10분이 다른 봉과 똑같이 나오던 이유 ═══════════════════
         야후가 주는 간격은 1m·2m·5m·15m·30m·60m·90m 뿐이다. 3분과 10분은 없다.
         그래서 앞 판에서는 3분→5m, 10분→15m 을 '대신' 보여 줬는데,
         그러면 5분·15분 화면과 그림이 완전히 같아 "눌러도 안 바뀐다"가 된다.
         [고침] 없는 간격은 1분봉을 받아 직접 묶어서 만든다.
         3분이면 1분봉 3개를, 10분이면 10개를 하나로 합친다
         (시가=첫 봉의 시가, 고가=최댓값, 저가=최솟값, 종가=마지막 종가, 거래량=합).
         묶는 기준은 '분 단위 절대 시각'이라 매일 같은 자리에서 끊긴다. */
      /* ══ [v12.7] 야후가 주는 간격만 쓴다 ═══════════════════════════════════
         야후 chart API 가 제공하는 분 간격은 1m · 2m · 5m · 15m · 30m · 60m 이다.
         없는 간격(3분·10분)을 봉을 묶어 만들어 보았으나, 원본을 손대는 만큼
         값이 실제 거래소 봉과 어긋날 여지가 생긴다. 시세 화면에서 그건 위험하다.
         그래서 만들지 않고, 받는 그대로만 보여 준다.
         화면의 기간 버튼도 이 목록에 맞춰 1·2·5·15·30·60분으로 바꿨다.
         [조회 가능 기간] 1분은 7일, 나머지 분봉은 60일까지가 야후 상한이다. */
      const MAP = {
        "1M":  ["7d",  "1m"],
        "2M":  ["1mo", "2m"],
        "5M":  ["1mo", "5m"],
      ...
      /* ══ [v12.7] 연봉만은 묶는다 ═══════════════════════════════════════════
         야후에 '1년' 간격이 없다. 분봉과 달리 연봉은 묶어도 값이 어긋날 여지가
         없다 — 한 해의 첫 시가·최고·최저·마지막 종가는 정의가 하나뿐이기 때문이다.
         (분봉은 거래소가 정한 봉 경계와 다르게 끊길 수 있어 만들지 않는다) */
      if (tf === "Y" && c.length) {
        const byY = new Map();
        c.forEach(k => {
...
var APP_VER = "17.7.1";  /* version-info.js 의 version 과 반드시 일치시켜야 한다 */
var worker_default = {
